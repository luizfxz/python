...
def alteracao(): 
    ...
    consulta_sql= "SELECT * FROM cd_user where nome= '{}'".format(login)
    cursor = connect.cursor()
    ...
    for linha in linhas:
        ...
        id = alt_dados.id.setText(str(linha[0]))
...
def excluir_dados_tabela_produtos(): 
...
def alteracao_tabela_funcionarios(): 
    
    alter_func.show()
    

    cursor = connect.cursor()
    sql = "Select * from cd_func"
    cursor.execute(sql)

    leitura_sql = cursor.fetchall()

    alter_func.tableWidget.setRowCount(len(leitura_sql)) #tabela de clientes
    alter_func.tableWidget.setColumnCount(9) # colunas preenchidas na lista de cliente

    for i in range(0, len(leitura_sql)):
        for j in range(0, 9):
            alter_func.tableWidget.setItem(i, j, QtWidgets.QTableWidgetItem(str(leitura_sql[i][j]))) # preenchimento das linhas e colunas

def excluir_dados_tabela_funcionarios(): 

    linha =  alter_func.tableWidget.currentRow() # linha selecionada
    alter_func.tableWidget.removeRow(linha) # remoção da linha

    cursor = connect.cursor()
    cursor.execute('SELECT id FROM cd_func')
    execute_sql = cursor.fetchall()
    valor_id = execute_sql[linha][0]
    cursor.execute("DELETE FROM cd_func WHERE id = " + str(valor_id))
...
def edicao_dados_tabela_funcionarios(): 

    cd_opções.hide()

    linha =  alter_func.tableWidget.currentRow()
    ...
    execute_sql = cursor.fetchall()
    valor_id = execute_sql[linha][0]
    cursor.execute("SELECT * FROM cd_func WHERE id = " + str(valor_id))
    editar = cursor.fetchall()

    edit_func.show()

    edit_func.func_cod.setText(str(editar[0][0]))
    edit_func.func_nome.setText(str(editar[0][1]))
    edit_func.func_email.setText(str(editar[0][2]))
    edit_func.func_senha.setText(str(editar[0][3]))
    edit_func.func_cep.setText(str(editar[0][4]))
    edit_func.func_tel.setText(str(editar[0][5]))
    edit_func.func_ende.setText(str(editar[0][6]))
    edit_func.func_nasc.setText(str(editar[0][7]))
    edit_func.func_cpf.setText(str(editar[0][8]))


def salvar_dados_editados_tabela_funcionarios():


    cods = edit_func.func_cod.text()
    nomes = edit_func.func_nome.text()
    senhas = edit_func.func_senha.text()
    nascs = edit_func.func_nasc.text()
    cpfs = edit_func.func_cpf.text()
    ceps = edit_func.func_cep.text()
    enderecos = edit_func.func_ende.text()
    telefones = edit_func.func_tel.text()
    emails = edit_func.func_email.text()    
    ...
    atualizar = "UPDATE cd_func SET nome = '{}', senha = '{}', nasc = '{}', cpf = '{}', cep = '{}', endereco = '{}', telefone = '{}', email = '{}' where id = '{}' ".format (nomes,senhas,nascs, cpfs,ceps,enderecos, telefones, emails,cods)
    cursor.execute(atualizar)

    QtWidgets.QMessageBox.about(edit_func, 'Sucesso', 'Atualização feito com sucesso!')
...
# |=------------------------------------------------------------FUNÇÕES DE tabela de fornecedores ---------------------------------------------------------------------------------------=| #
...
alter_func = uic.loadUi('alter_func.ui')

edit_func = uic.loadUi('edit_func.ui')

alter_forne = uic.loadUi('alter_forne.ui')

edit_forne = uic.loadUi('edit_forne.ui')
...
cd_opções.exc_func.clicked.connect(alteracao_tabela_funcionarios)
...
catalogo.cat_voltar.clicked.connect(voltar_catalogo)

# |=------------------------------------------------------------------------TELA USUARIO--------------------------------------------------------------------------------=| #

catalogo.user.clicked.connect(alteracao) #Botão para chamar ediçao de dados
...
alter_prod.prod_excluir.clicked.connect(excluir_dados_tabela_produtos) # botão de excluir na tela de tabela de clientes

alter_prod.prod_alterar.clicked.connect(edicao_dados_tabela_produto) # botão de abrir a edição de dados 

edit_prod.prod_salvar.clicked.connect(salvar_dados_editados_tabela_produto) # botão de salvar na janela de alteração de dados 

# |=--------------------------------------------------------------------------------------------------------------------------------------------------------------------=| #

# |=-----------------------------------------------------------------Janela da tabela de funcionários --------------------------------------------------------------------------------------=| #



alter_func.func_excluir.clicked.connect(excluir_dados_tabela_funcionarios) # botão de excluir na tela de tabela de clientes

alter_func.func_alterar.clicked.connect(edicao_dados_tabela_funcionarios) # botão de abrir a edição de dados 

edit_func.func_salvar.clicked.connect(salvar_dados_editados_tabela_funcionarios) # botão de salvar na janela de alteração de dados 
